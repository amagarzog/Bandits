...
template <typename R>
typename GaussianProcessRegression<R>::VectorXr GaussianProcessRegression<R>::DoRegression(const VectorXr& inp, Eigen::MatrixXd kernel, bool prepare) {
    // if(prepare || b_need_prepare_){
    //   PrepareRegression();
    // }
    // can return immediately if no training data has been added..
    ...
    outp.setZero();
    //KXx = SQEcovFunc(input_data_, inp);
    KXx = KernelCovFunc(input_data_, inp, kernel);
...
template <typename R>
typename GaussianProcessRegression<R>::MatrixXr GaussianProcessRegression<R>::KernelCovFunc(const MatrixXr& x1, const MatrixXr& x2, const Eigen::MatrixXd& kernel) {
    MatrixXr retMat(x1.cols(), x2.cols());
    for (int i = 0; i < x1.cols(); i++) {
        for (int j = 0; j < x2.cols(); j++) {
            retMat(i, j) = kernel(i, j);
        }
    }
    return retMat;
}
